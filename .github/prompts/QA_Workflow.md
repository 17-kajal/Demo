...
This prompt guides you through a complete 7-step QA workflow using MCP
...
the jira: https://trivedikajal85.atlassian.net/browse/TD-1
...
Please review: 1. Test plan from: specs/saucedemo-checkout-test-plan.md
...
appropriate test suite files in: tests/saucedemo-checkout/ 3. Use the
...
Expected Output: - Test suite files created in tests/saucedemo-checkout/
...
updated in tests/saucedemo-checkout/ - Final stable test execution
...
Git Repository URL:
https://github.com/17-kajal/Demo.git
...
## STEP 8 - MANAGE JIRA PROJECT, TEST ARTIFACTS AND UPDATE TEST RESULTS:

Use the Atlassian MCP agent connected to https://trivedikajal85.atlassian.net/ to create and manage all Jira artifacts and update the testing task with final results.

Create a new Scrum project (if not already available) with appropriate project name, key, and Agile configuration.

Within the project, create a new Epic/Feature to group the functionality (e.g., E-commerce Checkout Flow) with proper title and description.

Create or verify the user story TD-1 under this Epic:

Add title, description, and acceptance criteria
Set priority, labels, and assignee if required
Link the user story to the Epic

Generate test cases from the user story:

Create individual test case issues
Include test steps, expected results, and test data
Cover happy path, negative scenarios, and edge cases

Link all test cases to the user story to maintain traceability (Epic → User Story → Test Cases).

Update the testing task in Jira with execution results:

Add test execution summary (PASS/FAIL/Blocked)
Include key findings from manual and automated testing
Add summary of defects identified
Provide final test status (Ready for Release / Blocked / Needs fixes)

Add a comment with:

Summary of testing performed
Reference to test plan and test report

Attach the test report file:

test-results/SCRUM-101-checkout-test-report.md

Update issue fields if required:

Status (e.g., QA Done / Ready for Release)
Labels (e.g., tested, automation-complete)

Notify relevant stakeholders (assignee, QA lead, developer).

Provide a final summary of:

Project, Epic, and User Story created/updated
Test cases generated and linked
Test results updated and report attached
